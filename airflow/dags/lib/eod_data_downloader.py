...
def download_massive_eod_data_to_csv(MASSIVE_API_KEY, LOOKBACK_DAYS):
    ...
    Arguments:
    - MASSIVE_API_KEY: API key for authentication with the Polygon API.
    ...
    MASSIVE_BASE_URL = 'https://api.massive.com'  # Base URL for Polygon API
    ...
    # Iterate through the past 'MASSIVE_MAX_LOOKBACK_DAYS' days to find a valid trading day
    for i in range(LOOKBACK_DAYS):
        ...
        try:
            ...
            log.info("[MASSIVE] %s -> %s", r.url, r.status_code)  # Log the request URL and response status
        except Exception as e:
            log.warning("[MASSIVE] request failed for %s: %s", trading_date, e)
            continue
